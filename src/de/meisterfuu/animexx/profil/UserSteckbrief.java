...
public class UserSteckbrief extends Activity  implements UpDateUI {
    ...
    private void LoadData(){
		...
		try {
			HTTPs[0] = Request.getHTTP("https://ws.animexx.de/json/mitglieder/steckbrief/?user_id="+userid+"&api=2");
	...
	public void UpDateUi(String[] s) {
		...
		try {
			...
			if(data.getBoolean("freigabe_steckbrief")){
				if(!data.isNull("foto_url"))Avatar.setImageDrawable(data.getString("foto_url"));
				if(!data.isNull("vorname")) vor = data.getString("vorname");
				if(!data.isNull("nachname")) vor += " "+data.getString("nachname");
				txtname.setText(vor);
				if(!data.isNull("geburtstag"))txtbirth.setText(data.getString("geburtstag"));
				if(!data.isNull("email"))txtmail.setText(data.getString("email"));
				if(!data.isNull("homepage"))txtweb.setText(data.getString("homepage"));
				if(!data.isNull("ort"))txtort.setText(data.getString("ort"));
				if(!data.isNull("alter"))txtalter.setText(data.getString("alter"));
			}		
	        ...
	        GB.setOnClickListener(new View.OnClickListener() {
	            public void onClick(View v) {
	        		...
	        		Intent newIntent = new Intent(getApplicationContext(), GBViewList.class);
