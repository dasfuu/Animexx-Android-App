...
public  class Request {
	...
	public static int[] GetUser(String[] Names) throws Exception{
		...
		String jsonOutput = "";
    		...
    		String s="";
    		s += "dummy=dummy";
    		for(int i = 0;i < Names.length; i++) s += "&users[]="+Names[i];
	...
	public static boolean sendENS(String Betreff, String Text, String Signatur, int[] an, String referenz_id) throws Exception {
		...
		s += "&sig="+Signatur;
		for(int i = 0;i < an.length; i++) s += "&an_users[]="+an[i];
        if(referenz_id != "-1"){
	...
	public static boolean checkpush(){	
		try {
			String jsonOutput = makeSecuredReq("https://ws.animexx.de/json/cloud2device/registration_id_get/?api=2");
	     	...
	     	if (jsonResponse.equals(config.getString("c2dm", "x"))) return true; else return false;
		} catch (Exception e) {
			return false;
		}
	}
