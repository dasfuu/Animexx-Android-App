...
public class GBViewList extends ListActivity {
	    ...
	    private GBObject[] getGBlist(){
	    	try {    		
	    		JSONObject jsonResponse = new JSONObject(Request.makeSecuredReq("https://ws.animexx.de/json/mitglieder/gaestebuch_lesen/?user_id="+id2+"&text_format=html&api=2"));
	    		...
	    		for(int i=0;i < GBlist.length();i++){
	    			...
	    			GBa[i] = new GBObject(tempo.getString("text_html"), tempo.getString("id"), tempo.getJSONObject("von").getString("username"), tempo.getJSONObject("von").getString("id"), tempo.getString("datum_server"), tempo.getString("avatar"));
	    		}}
	    		else return new GBObject[] {new GBObject()}; 
	    		temp = GBa;
	    		return temp;
			} catch (Exception e) {
				e.printStackTrace(); 
