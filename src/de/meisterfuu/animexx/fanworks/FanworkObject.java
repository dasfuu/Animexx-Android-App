...
public class FanworkObject {
	 ...
	 * - item_image_big: string / null: Absolute URL einer Abbildung, oder "null", falls keines vorhanden ist. Wird nur zurückgeliefert, wenn beim
	 * Aufruf img_max_x, img_max_y, img_quality und img_format angegeben wird.
	 * - beschreibung: string: HTML-Formatierte Beschreibung des Fanworks. Wird nur bei /fanworks/get_fanwork_data/ zurückgeliefert.
	 * - adult: boolean: Ist das Werk adult? Achtung: bei Doujinshis und Fanfics können einzelne Kapitel auch Adult sein, wenn das Hauptwerk
	 * Nicht-Adult ist.
	 * - hat_kommentare: boolean
	 * - kommentare_display: int (Wird nur zurückgegegeben, wenn hat_kommentare === true). Die Gesamtzahl der Kommentare abzüglich der Kommentare des
	 * Autors. Die Zahl, die auf der Website als Zahl angegeben wird.
	 * - kommentare_anz: int (Wird nur zurückgegegeben, wenn hat_kommentare === true). Anzahl der Kommentare auf das Werk, inkl. der Kommentare des
	 * Autors, aber ohne die Antworten.
	 * - kommentare_anz_antworten: int (Wird nur zurückgegegeben, wenn hat_kommentare === true). Anzahl der Antworten auf Kommentare zum Werk, inkl.
	 ...
	 * kommentar_*-Parametern zurückgeliefert.
	 */
